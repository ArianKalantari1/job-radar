# Prompt 06 — Slack Digest (`digest.py`)
## Context
You are working on the `job-radar` project. Read `scraper.py`, `scorer.py`, `server.py`, and `role_profiles.json` before writing this file.

The digest sends Slack messages twice daily. Jobs are grouped by `primary_role` — one Slack message per role family that has new jobs. Each message includes match reasoning, salary, and thumbs-up/thumbs-down buttons that call back to `server.py` endpoints.

## Task — Create `digest.py`

Create a new file `digest.py` in the project root:
...
## Slack App Setup (manual steps — Copilot cannot do this)
Before `digest.py` will work, you need to create a Slack app:

1. Go to https://api.slack.com/apps → "Create New App" → "From scratch"
2. Name: `job-radar` | Workspace: your personal workspace
3. Under "OAuth & Permissions" → "Bot Token Scopes", add:
   - `chat:write`
   - `links:write` (for link unfurling in buttons)
4. Click "Install to Workspace" → copy the **Bot User OAuth Token** → add to `.env` as `SLACK_BOT_TOKEN`
5. Invite the bot to your channel: in Slack, type `/invite @job-radar`
6. Get your channel ID: right-click the channel → "View channel details" → copy ID at the bottom → add to `.env` as `SLACK_CHANNEL_ID`

## Validation
First test without Slack credentials:
```bash
python digest.py --dry-run
```
Expected: prints job list grouped by role, no API calls made.

Then, after Slack app setup:
```bash
python digest.py
```
Expected: one Slack message per role family that has new jobs.

## Do NOT
- Do not modify `scraper.py`, `scorer.py`, `server.py`, or `run.py`
- Do not implement reaction *handling* here — that is `server.py`'s job
- Do not use `slack_sdk.WebhookClient` — use `WebClient` with Bot Token for better reliability
